...
Student name: ` Phobtham Ngao-Ngam `
...
1. If `Cm_alpha < 0` and the angle-of-attack disturbance is positive, `delta_Cm` should be `negative` because `positive aoa makes a nose down pitch`.
2. If `Cm_alpha > 0` and the angle-of-attack disturbance is positive, the response should be `positive` because `same pitch moment direction`.
3. If `Cm_alpha = 0`, changing angle of attack should `not change`.
4. If `Cm0` is fixed and the magnitude of a nonzero `Cm_alpha` increases, the trim angle magnitude should `decrease`.
5. Doubling `disturbanceAlphaDeg` while holding `Cm_alpha` fixed should `double delta_Cm`.
...
Angle conversion:
alpha_rad = 2.86 * pi/180 = 0.0499 rad
delta_alpha_rad = 2.00 * pi/180 = 0.0349 rad

Current pitching-moment coefficient:
Cm(alpha) = Cm0 + Cm_alpha * alpha_rad = 0.04 + 0.8 * 2.86 * pi/180 = 0.0799

Trim angle:
alpha_trim_rad = -Cm0/Cm_alpha = -0.04/0.8 = -0.05 rad
alpha_trim_deg = alpha_trim_rad * 180/pi = -0.05 * 180/pi = -2.8648 deg

Disturbance response:
delta_Cm = Cm_alpha * delta_alpha_rad = 0.8 * 2.00 * pi/180 = 0.0279
...
input: Cm0, Cm_alpha, angleofAttackDeg, disturbanceAlphaDeg
output: Cm = Cm0 + Cm_alpha * angleofAttackDeg * pi/180 = 0.04 + 0.8 * 2.86 * pi/180 = 0.0799
        alpha_trim_rad = -0.0500 rad
        alpha_trim_deg = -2.8648
        delta_Cm = Cm_alpha * delta_alpha_rad = 0.02793
relationships: not trimmed and destabilizing
tolerance: 0.0001 plus or minus
...
increase Cm_alpha from negative towards zero while delta_alpha is positive, delta_Cm must increase towards zero
...
zero slope: Cm_alpha = 0 1/rad, Cm0 and alpha is fixed
delta_Cm = 0, no trim angle should be reported because it has to be divided by zero
...
It will support desicions about whether the given condition is trimmed and aoa condition. It cannot establish overall stability or real behavior.
